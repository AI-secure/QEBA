...
if __name__ == '__main__':
    ...
    inp = graph.get_tensor_by_name('data:0')
    outp = graph.get_tensor_by_name('prob:0')
    ###
    ##outp = graph.get_tensor_by_name('prob:0')
    #outp = graph.get_tensor_by_name('conv6_sep_bn/Relu:0')
    ##outp = graph.get_tensor_by_name('conv5_6_sep_bn/Relu:0')
    ##outp = graph.get_tensor_by_name('conv4_2_sep_bn/Relu:0')
    ##outp = graph.get_tensor_by_name('conv3_2_sep_bn/Relu:0')
    ##outp = graph.get_tensor_by_name('conv2_2_sep_bn/Relu:0')
    ##outp = graph.get_tensor_by_name('conv1_bn/Relu:0')
    ##for tensor in graph.as_graph_def().node:
    ##    print (tensor.name)
    #for op in graph.get_operations():
    #    print (str(op.name))
    #image, _ = get_image_data('../raw_data/receipt/imgs/receipt/2F201805151100300604700503679351_1_1527153358212.jpg')
    ...
    src_image, _ = get_image_data('../raw_data/receipt/imgs/receipt/2F201805151100300604700503679351_1_1527153358212.jpg')
    #tgt_image, _ = get_image_data('../raw_data/receipt/imgs/common_paper/2F201805181100300604210503668198_18_1526996089599.jpg')
    tgt_image, _ = get_image_data('../raw_data/imagenet_example/awkward_moment_seal.png')
    with tf.Session(graph=graph) as sess:
    ...
    #p_gen = UNet(n_channels=3, preprocess = (transp, mean, std))
    #p_gen.load_state_dict(torch.load('unet.model', map_location='cpu'))
    p_gen = UNet(n_channels=3, preprocess = (transp, mean, std), mobilenet=True)
    p_gen.load_state_dict(torch.load('unet_mobile.model', map_location='cpu'))
    ATK_LVL = 999
    ...
    with tf.Session(graph=graph) as sess:
        ...
        adv = attack(tgt_image, tgt_label, starting_point = src_image, iterations=64, stepsize_search='geometric_progression', verbose=True, unpack=False, max_num_evals=800, initial_num_evals=20, internal_dtype=np.float32, rv_generator = p_gen, atk_level=ATK_LVL, batch_size=1)
