    ...
    * optional multithreading for candidate point generation
    ...
    def _apply_inner(
        ...
        for step in range(1, iterations + 1):
            ...
            if self.has_converged(check_strict):
                self.log_step(step - 1, distance, always=True, a=a)
                if resetted:
                    self.printv(
            ...
            self.log_step(step, distance, message, a=a)
            sys.stdout.flush()
    ...
    def log_step(self, step, distance, message='', always=False, a=None):
        ...
        if a is not None:
            ...
            fig = plt.figure()
            plt.imshow(a.perturbed[:,:,::-1]/255)
            plt.axis('off')
            plt.title('Call %d'%a._total_prediction_calls)
            fig.savefig('BA_result/step%d.png'%step)
            print ("Call:", a._total_prediction_calls, "Saved to", 'BA_result/step%d.png'%step)
